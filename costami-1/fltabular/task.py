...
def _get_dataset_path(split: str) -> Path:
    ...
    default_name = "train_db.csv" if split == "train" else "val_db.csv"
    ...
    if not candidates:
        raise FileNotFoundError(
            f"No .csv file found in {Path.cwd()}. Set {env_name} or place {default_name} next to the app."
        )

    raise RuntimeError(
        f"Found multiple .csv files in {Path.cwd()}. Set {env_name} to the file path."
    )
...
def get_input_dim() -> int:
    dataset = pd.read_csv(_get_dataset_path("train"))
...
def _load_split_dataframe(split: str) -> tuple[pd.DataFrame, str]:
    dataset = pd.read_csv(_get_dataset_path(split))
